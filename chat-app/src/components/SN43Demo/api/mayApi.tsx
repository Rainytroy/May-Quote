...
async sendChatMessage(params: ChatParams): Promise<ChatResponse> {
    try {
      ...
      console.log('发送聊天消息，使用模型:', modelId);
      ...
      try {
        ...
        // 使用OpenAI客户端发送请求
        console.log('使用OpenAI客户端发送聊天请求...');
        const completion = await client.chat.completions.create({
          messages,
          model: modelId,
          temperature: 0.7,
          stream: false // 固定为非流式，简化处理
        });
        
        const content = completion.choices[0]?.message?.content || '无结果';
        const finishReason = completion.choices[0]?.finish_reason;
        
        console.log('成功收到聊天响应，内容长度:', content.length);
        ...
        console.error('OpenAI API聊天请求失败:', openaiError);
        throw openaiError;
      }
    } catch (error) {
